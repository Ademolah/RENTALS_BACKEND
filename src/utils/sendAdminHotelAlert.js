...
/**
 * Dispatches an internal operational alert to Super Admins for new hotel applications
 * @param {string} adminEmail - The destination address of the SUPERADMIN
 * @param {Object} hotelData - The newly created hotel application document
 * @param {Object} applicantData - The user who submitted the application
 */
export const sendAdminHotelAlert = async (adminEmail, hotelData, applicantData) => {
  try {
    ...
    // Safely extract hotel application data with fallbacks
    const businessName = hotelData.businessName || 'N/A';
    const cacNumber = hotelData.cacNumber || 'N/A';
    const locationInfo = `${hotelData.registeredAddress || 'N/A'}, ${hotelData.state || 'N/A'}`;
    const contactMatrix = `${hotelData.contactEmail || 'N/A'} • ${hotelData.contactPhone || 'N/A'}`;
    const applicantName = `${applicantData.firstName || ''} ${applicantData.lastName || ''}`.trim() || 'N/A';
    ...
    <head>
      ...
      <title>Admin Alert: New Hotel Registration</title>
      <style>
    ...
    <body style="margin:0; padding:0; background-color:#0F172A; color:#E2E8F0; -webkit-font-smoothing:antialiased;">
      ...
      <div style="display:none; font-size:1px; color:#0F172A; line-height:1px; max-height:0px; max-width:0px; opacity:0; overflow:hidden;">
        Action Required: A new hotel entity application for ${businessName} is pending review.
      </div>
        ...
        <tr>
          <td style="padding:30px 40px; background: #0F172A; border-bottom: 1px solid rgba(255,255,255,0.05);">
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td valign="middle">
                  <span style="color:#FFFFFF; font-weight:800; font-size:22px; tracking-tight;">Rentals<span style="color:#38BDF8;">
                </td>
        ...
        <tr>
          <td style="padding:40px;">
            ...
            <p style="margin:16px 0 0 0; font-size:15px; line-height:1.6; color:#94A3B8;">
              A new asset owner has submitted their corporate hotel details to join the network. Please review the documentation ledger and evaluate their verified status.
            </p>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="margin: 32px 0; background: #0F172A; border-radius: 12px; border: 1px solid rgba(255,255,255,0.05); overflow: hidden;">
              <tr>
                <td style="padding: 20px; border-bottom: 1px solid rgba(255,255,255,0.05);">
                  <p style="font-size:12px; font-weight:700; text-transform:uppercase; letter-spacing:1px; color:#64748B; margin-bottom:4px;">Hotel Entity</p>
                  <p style="font-size:16px; font-weight:600; color:#FFFFFF;">${businessName}</p>
                </td>
              </tr>
              <tr>
                <td style="padding: 20px; border-bottom: 1px solid rgba(255,255,255,0.05);">
              ...
              <tr>
                <td style="padding: 20px; border-bottom: 1px solid rgba(255,255,255,0.05);">
                  <p style="font-size:12px; font-weight:700; text-transform:uppercase; letter-spacing:1px; color:#64748B; margin-bottom:4px;">Registered Head Office</p>
                  <p style="font-size:15px; font-weight:500; color:#E2E8F0;">${locationInfo}</p>
                </td>
              </tr>
              <tr>
                <td style="padding: 20px; border-bottom: 1px solid rgba(255,255,255,0.05);">
                  <p style="font-size:12px; font-weight:700; text-transform:uppercase; letter-spacing:1px; color:#64748B; margin-bottom:4px;">Contact Channels</p>
                  <p style="font-size:15px; font-weight:500; color:#E2E8F0;">${contactMatrix}</p>
                </td>
              </tr>
              <tr>
    ...
    await transporter.sendMail({
      from: '"Rentals" <alerts@rentalsafrica.com>',
      to: adminEmail,
      subject: `Action Required: Hotel Registration - ${businessName}`,
      html: htmlContent,
    });
    
    console.log(`✅ Admin alert dispatched for ${businessName} to ${adminEmail}`);
  } catch (error) {
